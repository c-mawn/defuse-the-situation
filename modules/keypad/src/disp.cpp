...
// Button debouncing
const uint16_t DEBOUNCE_DELAY = 10; // in milliseconds
...
//   digitalWrite(2, LOW);
  ...
  if (pressed & (current == solution[counter])) {
    ...
    if (counter == 4) {
      ...
      display.drawBitmap(0, 0, solved_screen, 128, 64, SH110X_WHITE);
      display.display();
      digitalWrite(3, HIGH);
      while(1);
    }
  ...
  // Clear the buffer.
    ...
    display.clearDisplay();
    display.drawBitmap(0, 0, strike_screen, 128, 64, SH110X_WHITE);
    display.display();
    digitalWrite(2, HIGH);
	Serial.println(digitalRead(2));
    delay(1000);
